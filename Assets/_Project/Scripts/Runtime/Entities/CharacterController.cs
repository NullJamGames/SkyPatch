...
namespace NJG.Runtime.Entities
{
    ...
    public class CharacterController : ValidatedMonoBehaviour, ICharacterController
    {
        ...
        [field: FoldoutGroup("Jumping"), SerializeField]
        public bool AllowJumpingWhenSliding { get; private set; } = false;
        [field: FoldoutGroup("Jumping"), SerializeField]
        public bool AllowDoubleJump { get; private set; } = false;
        [field: FoldoutGroup("Jumping"), SerializeField]
        public bool AllowWallJump { get; private set; } = false;
        [field: FoldoutGroup("Jumping"), SerializeField]
        public float JumpSpeed { get; private set; } = 10f;
        [field: FoldoutGroup("Jumping"), SerializeField]
        public float JumpScalableForwardSpeed { get; private set; } = 10f;
        [field: FoldoutGroup("Jumping"), SerializeField]
        public float JumpPreGroundingGraceTime { get; private set; } = 0f;
        [field: FoldoutGroup("Jumping"), SerializeField]
        public float JumpPostGroundingGraceTime { get; private set; } = 0f;
        ...
        public void SetInputs(ref PlayerCharacterInputs inputs)
        {
            ...
            if (inputs.Interact)
            {
                if (Motor.CharacterOverlap(Motor.TransientPosition, Motor.TransientRotation, _probedColliders, LadderLayer,
                ...
                case CharacterState.Default:
                {
                    if (_lookInputVector.sqrMagnitude > 0f && OrientationSharpness > 0f)
                    {
                        ...
                        Vector3 smoothedLookInputDirection = Vector3.Slerp(Motor.CharacterForward, 
                            _lookInputVector, 1 - Mathf.Exp(-OrientationSharpness * deltaTime)).normalized;
                ...
                case CharacterState.Default:
                {
                        ...
                        if (_moveInputVector.sqrMagnitude > 0f)
                        {
                            ...
                            if (Motor.GroundingStatus.FoundAnyGround)
                            {
                                Vector3 perpenticularObstructionNormal = Vector3.Cross(Vector3.Cross(
                                    Motor.CharacterUp, Motor.GroundingStatus.GroundNormal), Motor.CharacterUp).normalized;
                    ...
                    // Handle jumping
                    {
                        ...
                        if (_jumpRequested)
                        {
                            ...
                            if (_canWallJump ||
                                (!_jumpConsumed && ((AllowJumpingWhenSliding ? Motor.GroundingStatus.FoundAnyGround 
