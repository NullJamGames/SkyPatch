    ...
    public abstract class PathRay : RaySensor
    {
        ...
        protected int AdvancePathCast(float startRadius, float radius = 0f)
        {
            if (radius > 0 || startRadius > 0)
            {
                ...
                {
                    var dir = PathPoints[i + 1] - PathPoints[i];
                    if (!Physics.SphereCast(PathPoints[i], Mathf.Lerp(startRadius, radius, (float) i / (PathPoints.Count - 2)), dir.normalized, out hit, dir.magnitude, detectLayer.value, triggerInteraction)) continue;
                    return i;
                }
            }
            else
